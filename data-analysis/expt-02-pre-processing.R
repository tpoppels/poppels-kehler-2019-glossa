...
grep.csv <- function(pattern,opts="-E", ...) {
    p <- pipe(sprintf("grep %s '%s' %s",opts,pattern,"../data/expt-02-raw-data"))
...
grep.csv(",AcceptabilityJudgment,.*,NULL,.*,NULL")[c(1,2,4,6,7,9,11)] -> r # extracting judgments
grep.csv(",AcceptabilityJudgment,.*,<b>.*</b>") -> sen # extracting sentences and merging them
r %>%
...
d %>% mutate(condition=ifelse(itemType=="experimental",
                              c("[P -> A]",
                                ...
                                "[P -> P]")[((ibexNo+1) %% 4) + 1],
                              NA),
             ellipsis=ifelse(itemType=="experimental",
                             rep(c(TRUE,FALSE),each=4)[((ibexNo+1) %% 8) + 1],NA)) -> d
...
d %>% mutate(fillerType = ifelse(ibexNo %in% c(199:210),"upper-bound elliptical",NA)) %>%
    mutate(fillerType = ifelse(ibexNo %in% c(211:222),"lower-bound elliptical",fillerType)) %>%
    mutate(fillerType = ifelse(ibexNo %in% c(223:234),"upper-bound non-elliptical",fillerType)) %>%
    mutate(fillerType = ifelse(ibexNo %in% c(235:246),"lower-bound non-elliptical",fillerType)) -> d
